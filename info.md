...
- **JAMstack部署平台**：JAMstack代表JavaScript、API和Markup（标记语言）。它是一种专门部署静态网站和前后端分离网站的平台。核心思想是**提前生成好所有的网页（静态文件），并通过通用的 API 来取代传统的数据库**，前后端解耦合。相较于传统部署方式，它不需要买服务器、不用配置Nginx、不用配置PM2，并且会跟随 Git Push 自动上线，拥有全球CDN等优势。
