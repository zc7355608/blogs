...
小程序开发框架提供丰富的微信原生 `API`，可以方便的调起微信提供的能力，如获取用户信息，本地存储，支付功能等，几乎所有小程序的 `API` 都挂载在 `wx` 对象底下，例如：`wx.chooseMedia()`、`wx.request()`, `wx` 对象实际上就是小程序的宿主环境微信所提供的**全局对象**。（`wx`类似浏览器中的`window`直接用就行）
...
1. 当接口参数 Object 对象中不包含 success/fail/complete 时将默认返回 promise，否则仍按回调方式执行，无返回值。
...
这时候就需要在小程序管理后台进行设置请求的域名，打开微信公众后台：点击左侧 开发 → 开发管理 → 开发设置 → 服务器域名。

**注意：request域名只支持 `https` 而且要求已备案**
...
将 **不校验合法域名、web-view (业务域名)、TLS版本以及HTTPS证书** 勾选上：（下图分别是模拟器、真机中的设置）
...
**注意**：提示的内容不会自动换行，如果提示的内容比较多，因为在同一行展示，多出来的内容就会被隐藏
...
> 2. 对象类型的数据存的时候没有使用转换，因此**获取的时候也不需要使用 `JSON.parse` 转换**
> 3. 其中`key`只能是string，value只支持原生类型、Date、及能够通过`JSON.stringify`序列化的对象。
> 4. storage 应只用来进行数据的持久化存储，不应用于运行时的数据传递或全局状态管理。启动过程中过多的同步读写存储，会显著影响启动耗时。
...
注意：**原生微信小程序”只支持 `?id=1&name=tom` 这种 Query String（查询字符串）格式**。
...
1. 在 app.json 或者 page.json 中配置距离页面底部距离`onReachBottomDistance`，默认 50px。即距离页面底部还有多少距离时，提前触发触底事件（`onReachBottom`）。
...
`bind:scrolltolower`：滚动到底部/右边时触发

`lower-threshold`：距底部/右边多远时，触发 scrolltolower 事件

`enable-back-to-top`：该布尔属性开启后，可以**让滚动条返回顶部**。iOS 可以点击顶部状态栏、安卓双击标题栏。注意：它只支持竖向滚动。
