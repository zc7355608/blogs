...
如果需要跳转到分包页面，需要在page路径之前添加上分包的根目录路径root：

```html
<navigator url="/modules/goodModule/pages/list/list"></navigator>
```
...
4. 分包之间**不能相互嵌套**，subpackage 的根目录不能是另外一个 subpackage 内的子目录



引用原则：

1. 主包不可以引用分包内的资源，但分包可以使用主包的公共资源。因为分包加载后主包资源肯定加载好了

2. 分包与分包之间资源无法相互引用。如果分包进行了异步化处理，则不受此条限制
...
开发者可以将功能相对独立的页面配置到独立分包中，因为**独立分包不依赖主包即可运行**，可以很大程度上提升分包页面的启动速度
...
开发者在`app.json`中，给`subpackages`定义的分包添加 `independent`字段，即可声明对应分包为独立分包。
...
```json
{
  ...
  "subPackages": [
    ...
    {
      "root": "modules/marketModule", // 该分包为独立分包
      "name": "marketModule",
      "pages": [
        "pages/market/market"
      ],
...
小程序的分包预下载需要在 `app.json` 中通过 `preloadRule` 字段设置预下载规则。`preloadRule` 是一个对象，对象的 `key` 表示访问哪个路径时进行预加载，`value` 是进入此页面的预下载配置对象，具有以下两个配置项：

| 字段     | 类型        | 必填 | 默认值 | 说明                                                         |
| -------- | ----------- | ---- | ------ | ------------------------------------------------------------ |
| packages | StringArray | 是   | 无     | 预下载的分包名称，进入页面后预下载分包的 `root` 或 `name`<br />`__APP__` 表示主包名称。 |
...
```json
{
  ...
  "preloadRule": {
    ...
    "modules/marketModule/pages/market/market": { // 对于独立分包，可以配置预下载主包
      "network": "all",
      "packages": ["__APP__"]
    }
  }
