...
建议将自定义组件放在根目录下的 `components` 文件夹中；如果该组件和页面有关联，也可以放在页面的目录中。
...
> 📌 **注意事项**：
>
> 1.  自定义组件需要在 `json` 文件中需要配置 `component` 字段设为 `true` 
...
`methods` 方法：在组件中用到的事件处理函数必须写到 `methods` 中
...
注意：和vue一样，**properties中的数据不建议修改**！虽然能够响应式，但会导致数据流的混乱。
...
默认情况下，一个组件的 wxml 中默认只能有一个 `<slot />` 。需要使用多 slot 时，要在组件 js 中通过`options.multipleSlots`声明启用。
...
类似于页面，自定义组件拥有自己的 `wxss` 样式，**组件 `wxss` 文件的样式，默认只对当前组件生效**。

编写组件样式时，需要注意以下几点：



1. app.wxss 或页面的 wxss 中使用了标签名（view）选择器（或一些其他特殊选择器）来直接指定样式，这些选择器会影响到页面和全部组件，通常情况下这是**不推荐的做法**。（和vue中类似）
   
2. 组件和引用组件的页面不能使用 id 选择器(#a)、属性选择器([a]) 和 标签名选择器，请改用 class 类选择器（标签选择器虽然能设置，但也不允许）

3. 组件和引用组件的页面中使用后代选择器（.a .b），可能出现一些非预期情况（比如组件设置了样式隔离），因此请**避免使用宽泛的、无其他约束条件的后代选择器**

4. **子元素选择器（.a>.b）只能用于 view 组件与其子节点之间**，不能用于其他节点结构，用于其他组件可能导致非预期的情况。
...
默认情况下，**自定义组件的样式只受到当前组件 wxss 的影响**。除非以下3种情况：



1. `app.wxss` 或页面的 `wxss` 中使用了标签名（view）选择器（或一些其他特殊选择器）来直接指定样式，这些选择器会影响到页面和全部组件。通常情况下这是不推荐的做法。

2. 从外部继承过来的样式（如font、color）。

3. 组件中指定**样式隔离选项** `styleIsolation` 
...
- `isolated` 表示启用样式隔离，在自定义组件内外，类选择器指定的样式将不会相互影响（一般情况下的默认值）；
...
> 
>
> 1. `.wx-checkbox-input {}`：复选框没有选中时默认的样式
> 2. `.wx-checkbox-input-checked {}`： 复选框选中时默认的样式
> 3. `.wx-checkbox-input.wx-checkbox-input-checked:before {}`：复选框选中时 √ 的样式
>
> 
>
>
...
>
> **但是在自定义组件内部使用的时候，需要开启样式隔离 `styleIsolation: 'shared'`**
>
> 开启了之后，此时组件中的样式会影响其他checkbox，因此最好让上方的类选择器结合该组件checkbox的自定义类名一块使用，将样式限制到该组件中。
...
数据监听器可以用于监听和响应任何属性`properties`和数据字段`data`的变化，有时，需要在一些数据字段被 setData 设置时，需要执行一些操作。那么就可以使用 `observers` 数据监听器来实现。语法如下：
...
在子组件中也可以通过 `this.setData()` 对 `properties` 中的数据进行修改，但是**一般不建议修改**
...
子组件如果需要向父组件传递数据，可以通过小程序提供的**事件**系统实现传递传递，可以传递任意数据。

事件系统是组件间通信的主要方式之一，自定义组件可以触发任意的事件，父组件的页面可以监听这些事件，流程如下：



1. 自定义组件触发事件时，需要使用 `triggerEvent` 方法发射一个自定义的事件。可以通过`event.detail` 获取子组件传递给父组件的数据
...
```html
<!-- 在自定义组件中 -->
<button type="primary" bind:tap="sendData">传递数据</button>
```
...
主要用于组件内部**监听父组件的展示、隐藏状态**，从而方便组件内部执行一些业务逻辑的处理
...
**小程序的页面也可以视为自定义组件，因此页面也可以使用 Component 方法进行创建**。

Component要比Page强大很多，从而实现复杂的页面逻辑开发。



> 📌 **注意事项：**
>
> 1. 要求对应 json 文件中包含 `usingComponents` 配置项
>2. 页面 Page 中的一些生命周期方法（如 onLoad() 等以“on”开头的方法），以及回调函数，在 Component 中要写在 methods 中才能生效
> 4. 组件的属性 properties 可以声明接收页面的query参数，在 onLoad() 中可以通过 this.data 拿到对应的页面参数
...
小程序的 behaviors 方法是一种代码复用的方式，可以将一些通用的逻辑和方法提取出来，然后在多个自定义组件中复用，从而减少代码冗余，提高代码的可维护性。（类似与vue的mixin）



**如果需要 behavior 复用代码，需要使用 Behavior() 方法**，每个 behavior 可以包含一组属性、数据、生命周期函数和方法

当自定义组件引用它时，它的属性、数据和方法会被合并到组件中，生命周期函数也会在对应时机被调用。
...
如果需要接收外部传的类名，需要**在 Component 中需要用 externalClasses 声明若干个外部样式类。**
...
2.自定义组件标签通过 properties 的方式提供一个样式类，属性名是 externalClasses 中定义的类名，属性值是要传递的具体类名
...
> 📌**注意事项：**
>
> - 在同一个节点上使用普通样式类和外部样式类时，两个类的优先级是未定义的；因此可以**添加 `!important` 以保证外部样式类的优先级**
> - 如果组件的样式隔离为`shared`，这时候 `externalClasses` 选项会失效。
