...
2. 当用户选择需要使用的头像之后，可以通过 `bind:chooseavatar` 事件回调获取到头像信息的临时路径。
...
```html
<view class="avatar">
  <button open-type="chooseAvatar" bind:chooseavatar="getAvatar">
...
注意：这个路径是一个临时路径，有过期时间的。在实际开发中，需要将这个临时图片保存在自己的服务器上。
...
2. 需要将 input 组件 type 的值设置为 nickname，当用户输入框输入时，键盘上方就会展示微信昵称了
...
1. 页面 js 文件 声明 `onShareAppMessage` 函数，它可以自定义转发内容。只有定义了该事件处理函数，右上角菜单才会显示“转发”按钮

2. 通过给 button 组件设置属性 open-type=“share“ ，在用户点击按钮后，也会触发 `onShareAppMessage` 事件监听函数
...
方式2：（如果不需要自定义转发的内容，`onShareAppMessage`写一个空函数就行）

```js
Page({

  // 监听页面按钮的转发 以及 右上角的转发按钮
  onShareAppMessage (obj) { // 通过button和右上角3个点转发，这两种方式的obj的form、target属性值是不同的
    // console.log(obj) // 右上角进行转发时，target是undefined，from是'menu'，button转发时from是'button'
...
Page({
  ...
  onShareTimeline () {
    ...
    return {
      // 自定义标题，即朋友圈列表页上显示的标题
      title: '帮我砍一刀~~~',
      // 自定义页面路径中携带的query参数，如 path?a=1&b=2 的 【 “?” 后面部分 】
      query: 'id=1',
...
1. 手机号快速验证组件（左图）：**平台会对号码进行验证，但不保证是实时验证**
...
2. 手机号实时验证组件（右图）：**在每次请求时，平台均会对用户选择的手机号进行实时验证**
...
> 📌注意事项：
>
> 1.目前该接口针对**非个人开发者**，且完成了认证的小程序开放（不包含海外主体）
>
...
2. 在微信公众后台，绑定后的客服账号，就可以登陆 [网页端客服](https://mpkf.weixin.qq.com/) 或 移动端小程序客服，进行接收、发送客服消息
