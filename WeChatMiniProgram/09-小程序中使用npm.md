...
>2. 我们在小程序中提到的包指专为小程序定制的 npm 包，简称**小程序 npm 包**，在使用包前需要先确定该包是否支持小程序
> 
...
在实际的开发中，随着项目的功能越来越多、项目越来越复杂，文件目录也变的很繁琐，为了方便进行项目的开发，开发人员通常会对目录结构进行调整优化，例如：将小程序源码放到 `miniprogram` 目录（类似`src`目录）下。
...
1. 新增 `miniprogramRoot` 字段，指定为调整后的小程序开发目录（小程序源码目录）
2. 新增 `setting.packNpmManually`设置为 `true`，允许自定义npm构建目录
    ...
    "packNpmRelationList": [
      {
        ...
        "miniprogramNpmDistDir": "./miniprogram"  //这里会将 miniprogram_npm 放在miniprogram目录中
      }
    ]
  }
}
...
1. 将小程序核心源码（app页面、pages、components、assets、static、sitemap.json等）放到 miniprogram 目录下
...
<!-- 需要相对于小程序源码的目录miniprogram来查找图片才可以 -->
...
1. 解除样式隔离：组件使用者直接在页面的样式文件中覆盖样式

   > 注意：如果是在组件中使用vant，则需要在组件中设置样式隔离为`shared`才可以覆盖其内部的样式。
