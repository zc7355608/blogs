...
微信运行时，会定期检查最近使用的小程序是否有更新。如果有更新，下次小程序启动时会同步进行更新，更新到最新版本后再打开小程序。如果 用户长时间未使用小程序时，启动小程序时会强制检查、更新后再启动。

> 基本小程序发布新版本24h后，99%的用户就可以完成更新。
>
...
App({
  ...
  onLaunch () {
    // 使用 wx.getUpdateManager 方法监听新版本下载的状态
