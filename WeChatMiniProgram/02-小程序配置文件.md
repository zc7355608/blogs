...
`pages` 字段：用于指定小程序由哪些页面组成（页面路由），用于为了让小程序知道由哪些页面组成以及页面定义在哪个目录，每一项都对应一个页面的 路径（含文件名） 信息。
...
> 3. 未指定 `entryPagePath: 'pages/index/index'` 时，数组的第一项就是小程序的首页
...
注意：窗口默认是不展示的，我们需要先开启`window.enablePullDownRefresh`，然后下拉才能看到窗口。
...
{
  "window": {
    "navigationBarBackgroundColor": "#f3514f", // 状态栏、导航条背景色
    "navigationBarTitleText": "慕尚花坊", // 导航条标题
    "navigationBarTextStyle": "white", // 导航条文字颜色
    "enablePullDownRefresh": true, // 开启下拉窗口
    "backgroundTextStyle": "light", // 窗口loading条的颜色
...
|       属性       |                             描述                             |  类型  | 是否必填 |
| :--------------: | :----------------------------------------------------------: | :----: | :------: |
|     pagePath     |       点击后要跳转到的页面路径，必须在 pages 中先定义        | string |    是    |
|       text       |                    每个 tab 按钮上的文字                     | string |    是    |
|     iconPath     | 未选中时的图片路径，icon 大小限制为 40kb，<br />建议尺寸为 81px * 81px， | string |    是    |
...
详细文档: [tabBar 配置项](https://developers.weixin.qq.com/miniprogram/dev/reference/configuration/app.html#tabBar)
...
其中每条规则都是一个对象，数组中可以配置多个对象。其中常用的配置项：
...
> 📌 **注意事项**：
>
> 1. 没有 sitemap.json 文件，则**默认所有页面都能被索引**
> 2. `{ "action": "disallow", "page": "*" }`是优先级最低的默认规则，未显式声明"disallow"的都默认被索引
