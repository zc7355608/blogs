...
小程序中绑定事件与在网页开发中绑定事件几乎一致，只不过在小程序不能通过 `on` 的方式绑定事件，也没有 `click` 等事件，小程序中**绑定事件使用 `bind` **，`click` 事件也需要**使用 `tap` 事件**来进行代替，绑定事件的方式有两种：
...
注意：字符串中的是一个函数名，不是JS表达式，因此不能写成函数调用的形式。
...
> 📌 **注意事项**：
>
> ​       除上表之外的**其他组件自定义事件，如无特殊声明都是非冒泡事件**
...
> 📌 **注意事项**：
>
> - 使用 `data-` 方法传递参数的时候，多个单词由连字符 `-` 连接。此时**连字符写法会转换成驼峰写法，而大写字符会自动转成小写字符**。例如：
>   - `data-element-type` ，最终会呈现为 `event.currentTarget.dataset.elementType `
>   - `data-elementType` ，最终会呈现为 `event.currentTarget.dataset.elementtype`
> - `event.currentTarget` 事件绑定者，也就是指：哪个组件绑定了当前事件处理函数；而 `event.target` 事件触发者，也就是指：哪个组件触发了当前事件处理函数。
